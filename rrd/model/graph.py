...
class DomeosGraph(object):
    def __init__(self, id, tmp_id, type, data):
        self.id = str(id)
        self.tmp_id = str(tmp_id)
        self.type = type
        self.data = data

    def __repr__(self):
        return "<DomeosGraph id=%s, tmp_id=%s, type=%s, data=%s>" %(self.id, self.tmp_id, self.type, self.data)
    __str__ = __repr__
